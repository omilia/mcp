...
import { askChoice, askSecret, askText } from "./prompts.js";
...
// ─── Client and auth choice labels (WIZ-02, WIZ-03) ─────────────────────────
...
const AUTH_CHOICES = [
  ...
  { value: "keycloak", label: "Keycloak username/password" }
];

/**
 * runWizard(options, io) — interactive orchestrator that collects ONLY the
 * fields returned by missingPromptFields(options), in PROMPT_FIELDS order,
 * then returns mergeFlagsAndAnswers(options, answers).
 *
 * Prompt routing:
 *   - "client"     → askChoice over CLIENT_CHOICES (WIZ-02)
 *   - "authChoice" → askChoice over AUTH_CHOICES (WIZ-03)
 *   - secret fields (accessToken, password) → askSecret (T-01-04)
 *   - all others   → askText (realm gets defaultValue "master")
 *
 * After authChoice is determined (from options or wizard answer), the required
 * credential fields are recomputed so only the PAT-specific or Keycloak-specific
 * fields are collected (WIZ-05: pre-supplied flags are never re-prompted).
 *
 * runWizard performs NO file writes and NO process spawning — it only returns
 * the completed options object (Phase 2 owns install execution, T-01-06).
 *
 * @param {object} options - Options object, may have pre-supplied flag values.
 ...
 * @returns {Promise<object>} The completed options object with all required fields.
 */
export async function runWizard(options, io = { input: process.stdin, output: process.stdout }) {
  const answers = {};

  // Helper: resolve current value — options (flags) first, then answers so far.
  ...
  // Determine whether a field still needs prompting, given current options+answers.
  function stillMissing(name) {
    return isMissing(currentValue(name));
  }

  // Process fields in PROMPT_FIELDS order.
  // We iterate once; for auth-conditional credential fields, we re-check
  // required each iteration using the accumulated answers so far.
  for (const field of PROMPT_FIELDS) {
    // Re-check what is required, incorporating authChoice resolved so far.
    const partialOptions = { ...options, ...answers };
    const resolvedAuth = partialOptions.authChoice;
    const required = requiredFieldsFor(partialOptions);

    // Determine if this field should be collected by runWizard.
    // requiredFieldsFor excludes realm (it has a default), but we still prompt
    // for realm in the keycloak path so users can override the "master" default.
    const isRealmForKeycloak = field.name === "realm" && resolvedAuth === "keycloak";
    const isRequired = required.includes(field.name) || isRealmForKeycloak;

    if (!isRequired) {
      continue;
    }

    // Skip fields already supplied as flags (WIZ-05).
    if (!isMissing(partialOptions[field.name])) {
      continue;
    }

    // Prompt for the field using the appropriate primitive.
    let value;

    if (field.name === "client") {
      value = await askChoice(field.label, CLIENT_CHOICES, io);
    } else if (field.name === "authChoice") {
      value = await askChoice(field.label, AUTH_CHOICES, io);
    } else if (field.secret) {
      value = await askSecret(field.label, io);
    } else {
      const opts = field.default !== undefined ? { defaultValue: field.default } : {};
      value = await askText(field.label, opts, io);
    }

    answers[field.name] = value;

    // After authChoice is determined, subsequent iterations will pick up the
    // auth-conditional credential fields via the updated partialOptions.
  }

  return mergeFlagsAndAnswers(options, answers);
}
