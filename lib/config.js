...
export const JSON_MCP_CLIENTS = new Set(["cursor", "claude"]);
// claude-code is a supported client but handled via `claude mcp add` CLI (CLI-02),
// not the JSON write path — so it is excluded from JSON_MCP_CLIENTS but remains
// in SUPPORTED_CLIENTS so the SUPPORTED_CLIENTS guard in finishInit accepts it.
export const SUPPORTED_CLIENTS = new Set([...JSON_MCP_CLIENTS, "claude-code", "codex", "vscode"]);
...
export function clientConfigPath(client, env = process.env) {
  ...
  // claude-code: no config-path write path — installed via `claude mcp add` CLI (CLI-02).
  // clientConfigPath("claude-code") intentionally returns undefined.
