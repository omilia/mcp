...
function runInit(args, io) {
  ...
  if (!SUPPORTED_CLIENTS.has(options.client)) {
    io.stderr.write(`Unsupported client: ${options.client}\n`);
    return 1;
  }

  const config = buildClientConfig(options.client, {
    ...
    realm: options.realm,
    useEnvVars: options.useEnvVars,
    authChoice: options.authChoice
  });

  if (options.write) {
    try {
      writeClientConfig(options.client, config, options);
      io.stdout.write(`Wrote ${options.client} MCP config to ${resolveWritePath(options.client, options)}\n`);
      return 0;
    } catch (error) {
      io.stderr.write(`${error.message}\n`);
      return 1;
  ...
  io.stdout.write(serializeClientConfig(options.client, config));
  return 0;
}
...
function parseInitOptions(args) {
  ...
  if (!options.client) {
    throw new Error("Missing required option: --client");
  }
