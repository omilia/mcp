...
function runInit(args, io) {
  ...
  // Resolve the input stream from EITHER the wizard-style `io.input` (injected by
  // tests) OR the real process stream `io.stdin`. bin/ocp-mcp.js passes `process`,
  // which exposes `stdin`/`stdout`/`stderr` — there is no `process.input`. Without
  // this fallback `io.input` is undefined in production, so the wizard never runs
  // and bare `init` falls straight to "Unsupported client: undefined". (production-TTY fix)
  const inputStream = io.input ?? io.stdin;
  const isTty = !!inputStream?.isTTY;
  const missing = missingPromptFields(options);

  // (WIZ-01) Interactive convenience: in a real terminal, collect any missing fields
  // via the wizard. `--print` is a pure "emit the config text" intent (the documented
  // Manual-config method) and must NEVER prompt — it emits placeholders for whatever
  // the user didn't supply.
  if (isTty && missing.length > 0 && !options.print) {
    // Async path — returns Promise<integer>.
    // runWizard expects { input, output }; map the resolved input stream + stdout.
    const wizardIo = { input: inputStream, output: io.stdout };
    return (async () => {
  ...
  // (WIZ-06) Non-interactive (no TTY) with no client: emit actionable guidance
  // instead of the cryptic "Unsupported client: undefined". The only HARD requirement
  // is --client; missing base-url/token are fine — they render as placeholders in
  // --print / --write (the documented manual + CLI-installer methods), preserving
  // legacy behavior. (A TTY with --print, or any call that supplies --client, falls
  // through to finishInit below.)
  if (!isTty && !options.client) {
