...
export function installClaudeCode(options, io, deps = {}) {
  ...
  try {
    ...
    // Surface claude's OWN error output (spawnSync captures stdout/stderr by default)
    // instead of swallowing it behind a bare exit code.
    const captured = result && (result.stderr || result.stdout);
    const detail = captured ? captured.toString().trim() : "";
    io.stderr.write(`claude mcp add exited with code ${exitCode}.\n`);
    if (detail) {
      io.stderr.write(`${detail}\n`);
    }
    // The most common cause is a name collision — claude mcp add refuses to overwrite.
    io.stderr.write(
      `If "${serverName}" is already registered, remove it and retry:\n` +
